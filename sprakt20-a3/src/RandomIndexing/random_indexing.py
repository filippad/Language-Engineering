...
class RandomIndexing(object):
    ...
    def __init__(self, filenames, dimension=2000, non_zero=100, non_zero_values=list([-1, 1]), left_window_size=2, right_window_size=2):
        ...
        self.__cv = defaultdict(list)
        self.__rv = defaultdict(list)
    ...
    def create_word_vectors(self):
        ...
        for word in self.__vocab:
            ...
            zeros = np.zeros(self.__dim - self.__non_zero)
            rv = np.append(rv, zeros)
        ...
        # Second pass through all files, update the context vector of each word
        for row in self.text_gen():
            for i, focus_word in enumerate(row):
                sum_rands = np.zeros(self.__dim)

                # Add left random vectors
                for dist in range(1, self.__lws):
                    ii = i - dist
                    if ii >= 0 and ii < len(row):    
                        word = row[ii]
                        sum_rands = sum_rands +  self.__rv[word]
                # Add right random vetors
                for dist in range(1, self.__rws):
                    ii = i - dist
                    if ii >= 0 and ii < len(row):    
                        word = row[ii]
                        sum_rands = sum_rands +  self.__rv[word]

                # Update context vector for the focus word        
                self.__cv[focus_word] = self.__cv[focus_word] + sum_rands
    ...
    def find_nearest(self, words, k=5, metric='cosine'):
        results = []
        values = list(self.__cv.values())
        keys = list(self.__cv.keys())
        neigh = NearestNeighbors(n_neighbors=k, metric='cosine')
        neigh.fit(values)

        for word in words:
            a = np.array(list(self.__cv[word])).reshape(1, -1)
    ...
    def train_and_persist(self):
        ...
        self.write_to_file()
        print("---Writing to file done in : %s seconds ---" % (time.time() - start_time))
    ...
    def write_to_file(self):
        ...
        with open("ri.txt", 'w') as f:
            ...
            for word, vector in self.__cv.items():
                if (word != ''):
                    f.write(word + " " + " ".join(map(lambda x: "{0:.6f}".format(x), vector)) + "\n")
...
if __name__ == '__main__':
    ...
    if args.cleaning:
        ri = RandomIndexing(['data/harry_potter_1.txt'])
