...
class Parser:
    ...
    def move(self, i, stack, pred_tree, move):
        ...
        if move == self.SH:
            ...
            top_ind = len(stack) - 1
            top_val = stack[top_ind]

            # create arc from the top most word to the second top most
            scnd_top_val = stack[top_ind - 1]
            pred_tree[scnd_top_val] = top_val

            # remove the second topmost word
            stack.remove(scnd_top_val)

        elif move == self.RA:
            top_ind = len(stack) - 1
            top_val = stack[top_ind]

            # create arc from the second top most word to the top most
            scnd_top_val = stack[top_ind - 1]
            pred_tree[top_val] = scnd_top_val

            # remove the topmost word
            stack.remove(top_val)
    ...
    def ok_to_remove(self, i_to_remove, i_first_unproccessed, correct_tree):
        if i_to_remove in correct_tree[i_first_unproccessed:]:
            return False
        return True
    ...
    def compute_correct_move(self, i, stack, pred_tree, correct_tree):
        ...
        options = self.valid_moves(i, stack, pred_tree)

        if len(stack) == 0 and self.SH in options:
            return self.SH
        ...
        if self.LA in options:
            to_put = stack[top_ind]
            if correct_tree[scnd_top_val] == to_put and self.ok_to_remove(scnd_top_val, i, correct_tree):
                return self.LA

        if self.RA in options:
            to_put = stack[top_ind - 1]
            if correct_tree[top_val] == to_put and self.ok_to_remove(top_val, i, correct_tree):
                return self.RA

        if self.SH in options and i < len(correct_tree):
            return self.SH
